...
function App() {
  ...
  async function openDeployment(deployment: Deployment) {
    ...
    try {
      ...
      setDetails((await response.json()) as DeploymentDetails);
    } catch (err) {
    ...
    return (
      <div className="app-shell">
        <aside className="sidebar">
          ...
          <nav>{['Overview', 'Deployments', 'Incidents', 'Services', 'AI Agent', 'Audit Logs'].map((item, i) => <div className={`nav-item ${i === 1 ? 'active' : ''}`} key={item}><span>{['⌂','↗','⚠','◈','✦','≡'][i]}</span>{item}</div>)}</nav>
          <div className="sidebar-footer">Production<br/><span>asia-south1</span></div>
        </aside>
        <main className="content">
          <header className="topbar">
            <div><button className="back-button" onClick={closeDeployment}>← Back to deployments</button><p className="eyebrow">DELIVERY EVIDENCE</p><h1>Deployment Details</h1></div>
          ...
          <section className="deployment-hero panel">
            <div><p className="eyebrow">GITHUB ACTIONS RUN</p><h2 className="deployment-title">{selectedDeployment.workflow}</h2><div className="deployment-identifiers"><span>Run #{selectedDeployment.id}</span><span>{selectedDeployment.branch}</span><span className="mono">{selectedDeployment.commit}</span><span>{selectedDeployment.environment}</span></div></div>
            <Status status={selectedDeployment.status}/>
          </section>
          <section className="panel evidence-panel">
            <div className="panel-header"><div><p className="eyebrow">EXECUTION EVIDENCE</p><h2>Jobs & steps</h2></div>{selectedDeployment.url && <a className="external-link" href={selectedDeployment.url} target="_blank" rel="noreferrer">Open GitHub run ↗</a>}</div>
            {detailsLoading ? <div className="empty-state">Loading GitHub Actions job evidence…</div> : details?.jobs.length ? details.jobs.map((job) => (
              <div className="job-card" key={job.id}>
                <div className="job-header"><div><strong>{job.name}</strong><span>Job #{job.id}</span></div><Status status={job.conclusion === 'success' ? 'SUCCESS' : job.status === 'completed' ? 'FAILED' : 'RUNNING'}/></div>
                <div className="step-list">{job.steps.map((step) => <div className="evidence-step" key={`${job.id}-${step.number}-${step.name}`}><span className={`step-icon ${step.conclusion === 'success' ? 'success' : step.conclusion === 'failure' ? 'failure' : 'pending'}`}>{step.conclusion === 'success' ? '✓' : step.conclusion === 'failure' ? '!' : '•'}</span><div><strong>{step.name}</strong><span>{step.status === 'completed' ? (step.conclusion || 'completed') : (step.status || 'pending')}</span></div></div>)}</div>
              </div>
          ...
          <section className="panel evidence-summary">
            ...
            <div className="evidence-grid"><Evidence label="Source" value="GitHub Actions" /><Evidence label="Workflow" value={selectedDeployment.workflow} /><Evidence label="Commit" value={selectedDeployment.commit} mono /><Evidence label="Environment" value={selectedDeployment.environment} /></div>
            <p className="evidence-note">This execution evidence is the input layer for the AI investigation pipeline. Diagnosis and remediation are intentionally separated from deployment execution.</p>
          </section>
        </main>
      </div>
    );
  ...
  return (
    <div className="app-shell">
      <aside className="sidebar">
        <div className="brand"><span className="brand-mark">AI</span><div><strong>CI/CD Platform</strong><small>Platform Engineering</small></div></div>
        <nav>{['Overview', 'Deployments', 'Incidents', 'Services', 'AI Agent', 'Audit Logs'].map((item, i) => <div className={`nav-item ${i === 0 ? 'active' : ''}`} key={item}><span>{['⌂','↗','⚠','◈','✦','≡'][i]}</span>{item}</div>)}</nav>
        <div className="sidebar-footer">Production<br/><span>asia-south1</span></div>
      </aside>
      <main className="content">
        ...
        <section className="grid-main">
          <div className="panel deployments"><div className="panel-header"><div><p className="eyebrow">DELIVERY</p><h2>Recent deployments</h2></div><button onClick={() => deployments[0] && openDeployment(deployments[0])}>View details →</button></div>
            <div className="table">{loading ? <div className="empty-state">Loading deployment data from GitHub Actions…</div> : deployments.length === 0 ? <div className="empty-state">No GitHub Actions deployments found.</div> : deployments.slice(0, 6).map((deployment) => <button className="row row-button" key={deployment.id} onClick={() => openDeployment(deployment)}><div><strong>{deployment.service}</strong><span>{deployment.workflow} · {deployment.branch} · {deployment.commit}</span></div><Status status={deployment.status}/><span className="muted">{deployment.time}</span></button>)}</div>
          </div>
          <div className="panel incident"><div className="panel-header"><div><p className="eyebrow">AI INVESTIGATION</p><h2>Active incident</h2></div><span className="severity">{activeIncident?.severity ?? '—'}</span></div>
            {loading ? <div className="empty-state">Loading incident data from FastAPI…</div> : activeIncident ? <>
              ...
              <div className="progress"><div className="progress-label"><span>Investigation progress</span><b>{activeIncident.status === 'RESOLVED' ? 'Resolved' : 'Evidence ready'}</b></div><div className="bar"><i style={{ width: activeIncident.status === 'RESOLVED' ? '100%' : '66%' }}/></div></div>
              <div className="steps"><Step done text="Incident received from backend"/><Step done text="GitHub Actions evidence collected"/><Step text={activeIncident.status === 'RESOLVED' ? 'No active failure to diagnose' : 'AI diagnosis available below'}/></div>
            </> : <div className="empty-state">No active incidents.</div>}
          </div>
        </section>
        {activeIncident && activeIncident.status !== 'RESOLVED' && <InvestigationPanel apiBaseUrl={API_BASE_URL} incidentId={activeIncident.id} />}
        <section className="panel activity"><div className="panel-header"><div><p className="eyebrow">OBSERVABILITY</p><h2>Platform activity</h2></div><span className="live"><i/> LIVE</span></div><div className="activity-list"><Activity time="LIVE" text="Deployment history is loaded from GitHub Actions"/><Activity time="LIVE" text="AI investigation is available for active incidents"/><Activity time="NEXT" text="GCP Cloud Run and Cloud Logging evidence are exposed through MCP"/></div></section>
      </main>
    </div>
  );
}
